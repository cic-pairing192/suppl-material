"""
Test pairings and group operations on BLS24 curves
Date: April 2023
...
def test_curve(u0):
    ...
    tw4x = (t4x - 3*y4x)//2
    px4 = px2**2
    assert ((px4 + 1 - tw4x) % rx) == 0
    assert c2x == (px4 + 1 - tw4x) // rx
    ...
    print("on G1")
    test_cofactor_clearing_g1(E, r, c, u0, t, None, cofactor_clearing_g1_bls24, verbose=False)
    test_membership_testing_g1(E, r, c, u0, t, -beta_mod_p-1, membership_testing_g1_bls24, verbose=False)
    print("on G2, M-twist")
    test_cofactor_clearing_g2(EM, r, c2, u0, t4, Fq6M, cofactor_clearing_g2_bls24, D_twist=False, verbose=False)
    test_membership_testing_g2(EM, r, c2, u0, t4, Fq6M, membership_testing_g2_bls24, D_twist=False, verbose=False)
    print("on G2, D-twist")
    test_cofactor_clearing_g2(ED, r, c2, u0, t4, Fq6D, cofactor_clearing_g2_bls24, D_twist=True, verbose=False)
    test_membership_testing_g2(ED, r, c2, u0, t4, Fq6D, membership_testing_g2_bls24, D_twist=True, verbose=False)
    ...
    test_final_exp_bls24_div3(Fp24D,r,u0,expected_exponent=ee//3)
    ...
    test_final_exp_bls24_div3(Fp24M,r,u0,expected_exponent=ee//3)
...
def test_curve_559():
    # BLS24-559 seed
    # seed from Barbulescu Duquesne 2019, but gives a BLS24-559, not aligned to machine-word-size
    u0 = ZZ(-2**56 - 2**43 + 2**9 - 2**6)
    test_curve(u0)

def test_curve_snark_502():
    # seed for SNARKs
    u0 = ZZ(2**50+2**48+2**44-2**41-2**38+1)
    assert u0 == 0x50dc000000001
    b = 1
    test_curve(u0)

def test_curve_snark_509():
    # seed for SNARKs
    u0 = ZZ(2**51+2**44+2**41-2**37+1)
    assert u0 == 0x811e000000001
    b = 1
    test_curve(u0)
...
if __name__ == "__main__":
    ...
    # read on the command-line the name-tag of the curve to test
    args = sys.argv
    if len(args) == 1: # no command-line parameter
        test_curve_315()
    ...
    while i < len(args):
        ...
        if ai == "315":
            ...
            test_curve_snark_502()
        elif ai == "snark509":
            test_curve_snark_509()
        i = i+1
