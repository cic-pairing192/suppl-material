"""
from the other project https://gitlab.inria.fr/zk-curves/snark-2-chains/
file sage/test_pairing_fst64.py
available online at
https://gitlab.inria.fr/zk-curves/snark-2-chains/-/blob/master/sage/test_pairing_fst64.py?ref_type=heads
"""
...
def test_curve_fst64_k20_574():
    ...
    assert u0 == -2**48+2**22-2**15-1
    a = 29
    cost_pairing_fst64_k20(u0)
    test_curve(k, u0, a)

def test_curve_fst64_k20_636():
    k = 20
    u0 = ZZ(-0x21fffffffffbff)
    assert u0 == -2**53-2**49+2**10+1
    a = 1
    cost_pairing_fst64_k20(u0)
    test_curve(k, u0, a)
