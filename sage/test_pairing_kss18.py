...
def test_lambda_omega_g1_j0(E, r, c, denom_c, u, lambr, lambc, omega):
    """
    test the correspondance of eigenvalue lambda mod r*c and primitive root omega in Fp, for E(Fp)
    the endomorphism is P -> (P[0]*omega, P[1])
    INPUT:
    - `E` : elliptic curve of j-invariant 0
    - `r`: prime order of G1
    - `c`: cofactor
    - `u`: seed for the parameters
    - `lambr`: eigenvalue of the endomorphism mod r
    - `lambc`: eigenvalue of the endomorphism mod c
    - `omega`: primitive cubic root mod p
    """
    Fp = E.base_field()
    p = Fp.characteristic()
    assert (lambr**2 + lambr + 1) % r == 0
    assert (lambc**2 + lambc + 1) % (c // denom_c) == 0
    assert (omega**2 + omega + 1) % p == 0
    ...
    while i < 10:
        P = E.random_element()
        P0 = denom_c*P
        psiP = E((P0[0]*omega, P0[1], P0[2]))
        if c*psiP == lambr*c*P0:
            lambr1_ok += 1
        elif c*psiP == (-lambr-1)*c*P0:
            lambr2_ok += 1

        if r*denom_c*psiP == r*denom_c*lambc*P0:
            lambc1_ok += 1
        elif r*denom_c*psiP == r*denom_c*(-lambc-1)*P0:
            lambc2_ok += 1
        i = i+1
    ...
    if lambr1_ok == i:
        ...
        print("                           eigenvalue of psi(P) mod r is -lambr-1")
    else:
    ...
    if lambc1_ok == i:
        ...
        print("                           eigenvalue of psi(P) mod c/{} is -lambc-1".format(denom_c))
    else:
...
def test_miller_loop_opt_ate_kss18(E, E2, Fqd, Fpk, map_Fqd_Fpk, r, c, c2, u, D_twist=False):
    ...
    or miller_loop_opt_ate_kss18_alt(Q2, P, u)
...
def test_curve(u0):
    ...
    (tx-2)**2 + 3*yx**2 == 4*rx*cx
    #g, inv_yx, _ = xgcd(yx, rx*cx)
    #sqrt_3 = (tx-2)*inv_yx % (rx*cx)
    #assert (sqrt_3**2 % (rx*cx)) == -3
    #lambd = (-1 + sqrt_3)/2
    lambx = (19*x**7 + 39*x**6 + 703*x**4 + 1786*x**3 + 6517*x + 19551)/343
    lambrx = x**3 + 18
    lambcx = x+2
    assert ((lambx**2 + lambx + 1) % (rx*cx)) == 0
    assert lambx % cx == x+2
    assert lambx % rx == x**3+18
    assert ((1 + (x+2 +1) * lambx) % cx) == 0
    ...
    lambr = ZZ(lambrx(u0))
    lambc = ZZ(lambcx(u0))
    test_lambda_omega_g1_j0(E, r, c, 343, u0, lambr, lambc, omega)
    print("c = {} = {}".format(c, c.factor()))
    g = gcd(p-1, c)
    print("gcd(c, p-1) = {} = {}".format(g, g.factor()))
    print("omega = {}".format(omega))
    test_cofactor_clearing_g1(E, r, c, u0, t, omega, cofactor_clearing_g1, verbose=False)
    test_cofactor_clearing_g1(E, r, c, u0, t, -omega-1, cofactor_clearing_g1_alt, verbose=False)

    print("test subgroup_membership_testing_g1")
    test_membership_testing_g1(E, r, c, u0, t, -omega-1, subgroup_membership_testing_g1, verbose=False)
    ...
    for case in range(euler_phi(k)):
        test_cofactor_clearing_g2(ED, r, c2, u0, t3, Fq6D, cofactor_clearing_g2, True, case)
    for case in range(euler_phi(k)):
        test_cofactor_clearing_g2(EM, r, c2, u0, t3, Fq6M, cofactor_clearing_g2, False, case)

    print("test subgroup_membership_testing_g2 with D-twist (2 tests), with M-twist (2 tests)")
    test_membership_testing_g2(ED, r, c2, u0, t3, Fq6D, subgroup_membership_testing_g2, D_twist=True)
    test_membership_testing_g2(ED, r, c2, u0, t3, Fq6D, subgroup_membership_testing_g2_alt, D_twist=True)
    test_membership_testing_g2(EM, r, c2, u0, t3, Fq6M, subgroup_membership_testing_g2, D_twist=False)
    test_membership_testing_g2(EM, r, c2, u0, t3, Fq6M, subgroup_membership_testing_g2_alt, D_twist=False)
    ...
    test_bilinearity_miller_loop_ate_absolute_extension(E, EM, Fp18M, Fp18M_A, map_Fp18M_Fp18M_A, r, c, c2, u0, D_twist=False, function_name=miller_loop_opt_ate_kss18_alt, curve_a_is_0=True)
    ...
    test_bilinearity_miller_loop_ate_absolute_extension(E, ED, Fp18D, Fp18D_A, map_Fp18D_Fp18D_A, r, c, c2, u0, D_twist=True, function_name=miller_loop_opt_ate_kss18_alt, curve_a_is_0=True)
    ...
    expected_exponent = ((p**6 - p**3 + 1)//r) * 3 *(21*(u0//7)**3 + 1)
    # 3*(3*u^3/49 + 1)*(q^6 - q^3 + 1)/r
    test_final_exp_hard_kss18(Fp18M_A, r, u0, function_name=final_exp_hard_kss18_v2, expected_exponent=expected_exponent)
...
if __name__ == "__main__":
    ...
    test_curve(u0)
    # a negative seed for testing
    u0 = ZZ(-2**44-2**37-2**34-2**28-2**23)
    test_curve(u0)
    # seed with high 2-valuation of (r-1)
    u0 = ZZ(0xc0c44000000)
    test_curve(u0)
    # seed for 192-bit security level 0x12fffdfdfffffffffc000
    u0 = ZZ(2**80 + 2**77 + 2**76 - 2**61 - 2**53 - 2**14)
    test_curve(u0)
    # seed from Barbulescu Duquesne 2019, but gives a KSS18-676, not aligned to machine-word-size
    #u0 = ZZ(-2**85 - 2**31 - 2**26 + 2**6)
    #test_curve(u0)
